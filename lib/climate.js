...
export async function fetchSiteClimate(lat, lng, { years = 5, timeoutMs = 12000 } = {}) {
  ...
  // Hard timeout so a slow/large archive response can never hang the caller —
  // the program build falls back to regional normals instead.
  const ctrl = new AbortController()
  const timer = setTimeout(() => ctrl.abort(), timeoutMs)
  let res
  try {
    res = await fetch(`${ARCHIVE_URL}?${params.toString()}`, { signal: ctrl.signal })
  } finally {
    clearTimeout(timer)
  }

...
// ── Fungicide spray log ──────────────────────────────────────────────────────
// A plain, honest RECORD instead of a computed "status": what fungicide went
// down, when, and what it typically covers — grouped by area, newest first — so
// the superintendent can read the recent history and judge coverage himself.
// No countdown, no red/amber/green: just the facts he entered, made legible.
//
// Returns one row per area: { area, lastDate, sprays: [ { date, since,
// products:[names], diseases:[labels] } ] }. Areas and sprays are newest-first.
export function fungicideLogByArea(sheets, products, asOf, { perArea = 3, lookbackDays = 60, diseaseCap = 6 } = {}) {
  const prodByName = {}
  ...
  // The diseases a set of products typically controls, deduped, common turf
  // diseases first (order comes from diseasesForProduct).
  const covered = (names) => {
    const seen = new Set()
    const out = []
    names.forEach((n) => diseasesForProduct(prodByName[n] || n).forEach((d) => {
      const k = d.toLowerCase()
      if (!seen.has(k)) { seen.add(k); out.push(d) }
    }))
    return out.slice(0, diseaseCap)
  }

  // Fungicide products, merged per area+date (so one spray day is one row even
  // if it came from two sheets).
  const byArea = {}
  ;(sheets || [])
    .filter((s) => sheetApplied(s) && s.date && s.area && s.date >= cutoff)
    .forEach((s) => {
      const fung = (s.products || []).filter((p) => isFungicide(p.product)).map((p) => p.product)
      if (fung.length === 0) return
      const perDate = (byArea[s.area] ||= {})
      const set = (perDate[s.date] ||= new Set())
      fung.forEach((p) => set.add(p))
    })

  return Object.keys(byArea)
    .map((area) => {
      const perDate = byArea[area]
      const sprays = Object.keys(perDate)
        .sort((a, b) => b.localeCompare(a))
        .slice(0, perArea)
        .map((date) => {
          const productsOnDay = [...perDate[date]]
          return { date, since: daysSince(date, asOfISO), products: productsOnDay, diseases: covered(productsOnDay) }
        })
      return { area, sprays, lastDate: sprays[0]?.date || '' }
    })
    .filter((r) => r.sprays.length > 0)

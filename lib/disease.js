...
// The diseases the last spray's products typically control, as a deduped list
// (best-rated first, common turf diseases surfaced first within each product).
// Lets the card say WHAT is covered, not just which products are in the tank.
function coveredDiseases(names, prodByName, cap = 6) {
  const seen = new Set()
  const out = []
  ;(names || []).forEach((n) => {
    diseasesForProduct(prodByName[n] || n).forEach((d) => {
...
export function protectionByArea(sheets, products, areas, asOf, tempSeries = null) {
  ...
  // Show every area that actually has a fungicide spray (keyed by the sheet's own
  // area name), so a slight name mismatch with Settings can't hide the bars.
  const areaKeys = Object.keys(lastByArea)

  const rows = areaKeys.map((area) => {
    const last = lastByArea[area]
    if (!last) return { area, last: null, status: 'none', remaining: null, window: null, since: null, pct: 0, mode: 'none', diseases: [] }
    ...
    const diseases = coveredDiseases(last.products, prodByName)
    ...
    if (bd) {
      ...
      return { area, last, since, window, mode: 'temp', remaining, pct: bd.pct, status, ddd: bd.ddd, threshold: bd.threshold, daysLeft: bd.daysLeft, rate: bd.rate, diseases }
    }
    ...
    return { area, last, since, window, mode: 'days', remaining, pct, status, diseases }
  })

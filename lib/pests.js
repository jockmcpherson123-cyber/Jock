...
export function airDiseaseRisks(daily = [], asOf, grasses = []) {
  ...
  const moisture = Math.max(ramp(rh, 78, 92), ramp(wetHrs, 5, 12))
  ...
  const models = [
    ...
    { id: 'pythium', label: 'Pythium blight', grasses: ['bent', 'poa', 'rye', 'bluegrass', 'fescue'], desc: 'Hot days (≥86°F), warm nights (≥68°F) and high humidity/leaf wetness. Can develop rapidly.', score: Math.round(Math.min(ramp(dayMax, 82, 90), ramp(nightMin, 65, 73), ramp(rh, 78, 90))) },
    ...
    { id: 'microdochium', label: 'Microdochium patch', grasses: ['bent', 'poa', 'rye', 'fescue', 'bluegrass'], desc: 'Favoured by cool (≤55°F), high humidity (≥85%) and prolonged leaf wetness/rain.', score: Math.round(Math.min(ramp(dayMax, 60, 45), ramp(rh, 82, 95))) },
  ]

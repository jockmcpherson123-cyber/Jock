...
export function airDiseaseRisks(daily = [], asOf, grasses = []) {
  ...
  const models = [
    ...
    { id: 'anthracnose', label: 'Anthracnose', grasses: ['poa', 'bent'], desc: 'Daytime heat stress (80–95°F) with prolonged leaf wetness on stressed annual bluegrass — worst late summer under low mowing / low nitrogen.', score: Math.round(Math.min(ramp(dayMax, 78, 92), moisture) * (0.7 + 0.3 * ramp(nightMin, 60, 72) / 100)) },
    ...
    { id: 'gray_leaf', label: 'Gray leaf spot', grasses: ['rye', 'fescue'], desc: 'Warm days (80–90°F), nights above ~65°F and prolonged humidity/leaf wetness in August; worst on perennial ryegrass.', score: Math.round(Math.min(ramp(dayMax, 80, 90), ramp(nightMin, 64, 72), moisture)) },
...
export function soilDiseaseRisks(soil, trend, asOf, grasses = []) {
  ...
  const models = [
    ...
    { id: 'goosegrass_germ', label: 'Goosegrass germination', grasses: ['any'], desc: 'Germinates in spring as soil warms through ~60–65°F, ~2 weeks after crabgrass. Apply pre-emergent before this window; the window closes once soil is warm and cooling.', score: rising ? inBand(soil, 58, 68) : inBand(soil, 58, 68) * 0.25 },
    ...
    { id: 'summer_patch', label: 'Summer patch', grasses: ['poa', 'bluegrass', 'fescue', 'bent'], desc: 'Root infection begins in spring as soil warms through ~64–70°F at 2" — the preventive fungicide window. Symptoms appear in summer, but the spraying window is spring.', score: rising ? inBand(soil, 62, 72) : inBand(soil, 62, 72) * 0.3 },
...
export const PEST_STAGES = [
  ...
  { id: 'crabgrass', label: 'Crabgrass germination', stages: [{ name: 'First germination (apply PRE before)', start: 120, end: 300 }, { name: 'Peak germination', start: 300, end: 600 }] },
  ...
  { id: 'white_grub', label: 'White grub egg hatch', stages: [{ name: 'Adult flight / egg-laying', start: 1030, end: 2000 }, { name: 'Egg hatch — young grubs (curative window)', start: 2000, end: 2600 }] },

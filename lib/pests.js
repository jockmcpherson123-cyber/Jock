...
export function airDiseaseRisks(daily = [], asOf) {
  ...
  const models = [
    ...
    { id: 'dollar_spot', label: 'Dollar spot', desc: 'Warm days (≥~70°F), mild humid nights with heavy dew, and prolonged leaf wetness — the classic warm-day/cool-humid-night pattern.', score: Math.round(Math.min(ramp(dayMax, 60, 77), ramp(nightMin, 46, 58), moisture)) },
    ...
    { id: 'pythium', label: 'Pythium blight', desc: 'Hot days (≥86°F), warm nights (≥68°F) and high humidity/leaf wetness. Can develop rapidly.', score: Math.round(Math.min(ramp(dayMax, 82, 90), ramp(nightMin, 65, 73), ramp(rh, 78, 90))) },

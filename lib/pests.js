...
// Forward-looking disease outlook for a report: scores the coming `days` of
// FORECAST weather (which has temps + rain chance but no humidity), using rain
// probability / amount as the leaf-wetness proxy in place of RH/dew point. Same
// published temperature thresholds as airDiseaseRisks. Returns worst-first,
// only rows scoring at least `minScore` (default: low band and up).
export function forecastWatch(daily = [], fromISO, grasses = [], days = 7, minScore = 15) {
  const rows = (daily || []).filter((d) => !fromISO || d.date >= fromISO).slice(0, days)
  if (rows.length === 0) return []
  const dayMax = mean(rows.map((d) => d.tMax))
  const nightMin = mean(rows.map((d) => d.tMin))
  const rainProb = mean(rows.map((d) => (d.precipProb != null ? d.precipProb : (d.precip > 0 ? 60 : 0))))
  const rainTot = rows.reduce((s, d) => s + (d.precip || 0), 0)
  const moisture = Math.max(ramp(rainProb, 25, 65), ramp(rainTot, 0.15, 0.9))
  const models = [
    { id: 'dollar_spot', label: 'Dollar spot', grasses: ['any'], score: Math.min(ramp(dayMax, 60, 77), ramp(nightMin, 46, 58), moisture) },
    { id: 'brown_patch', label: 'Brown patch', grasses: ['any'], score: Math.min(ramp(nightMin, 58, 68), ramp(dayMax, 78, 88), moisture) },
    { id: 'pythium', label: 'Pythium blight', grasses: ['bent', 'poa', 'rye', 'bluegrass', 'fescue'], score: Math.min(ramp(dayMax, 82, 90), ramp(nightMin, 65, 73), moisture) },
    { id: 'anthracnose', label: 'Anthracnose', grasses: ['poa', 'bent'], score: Math.min(ramp(dayMax, 78, 92), moisture) * (0.7 + 0.3 * ramp(nightMin, 60, 72) / 100) },
    { id: 'gray_leaf', label: 'Gray leaf spot', grasses: ['rye', 'fescue'], score: Math.min(ramp(dayMax, 80, 90), ramp(nightMin, 64, 72), moisture) },
    { id: 'microdochium', label: 'Microdochium patch', grasses: ['bent', 'poa', 'rye', 'fescue', 'bluegrass'], score: Math.min(ramp(dayMax, 60, 45), moisture) },
  ]
  return models
    .filter((m) => grassApplies(m.grasses, grasses))
    .map((m) => ({ id: m.id, label: m.label, score: clamp(Math.round(m.score)), band: band(clamp(Math.round(m.score))) }))
...
// Active ingredients that control each pest (lowercase), used to match the
// club's own Chemical Library so the app can say "you have this in stock"
// instead of a generic class. Kept separate from PEST_WATCH so the display
// copy and the matching keywords can evolve independently.
export const PEST_ACTIVES = {
  fall_armyworm: ['chlorantraniliprole', 'cyantraniliprole', 'tetraniliprole', 'spinosad', 'bacillus thuringiensis', 'indoxacarb', 'bifenthrin', 'lambda-cyhalothrin', 'deltamethrin'],
  white_grub: ['trichlorfon', 'clothianidin', 'imidacloprid', 'chlorantraniliprole', 'cyantraniliprole', 'thiamethoxam'],
  chinch_bug: ['bifenthrin', 'lambda-cyhalothrin', 'clothianidin', 'chlorantraniliprole', 'deltamethrin', 'carbaryl'],
  cutworm_webworm: ['chlorantraniliprole', 'spinosad', 'bacillus thuringiensis', 'indoxacarb', 'bifenthrin', 'lambda-cyhalothrin', 'deltamethrin'],
  abw: ['bifenthrin', 'lambda-cyhalothrin', 'cyantraniliprole', 'chlorantraniliprole', 'spinosad', 'indoxacarb'],
  billbug: ['bifenthrin', 'lambda-cyhalothrin', 'chlorantraniliprole', 'imidacloprid', 'clothianidin'],
  crabgrass: ['prodiamine', 'dithiopyr', 'pendimethalin', 'oxadiazon', 'benefin'],
}

// Find products in the club's Chemical Library whose active ingredient (or, as a
// fallback, product name) matches a pest's control chemistry. Returns a small
// list with stock so the UI can flag what's already on the shelf.
export function matchLibraryForPest(pestId, products = []) {
  const actives = PEST_ACTIVES[pestId]
  if (!actives || !actives.length) return []
  const norm = (s) => String(s || '').toLowerCase()
  return (products || [])
    .map((p) => {
      const ai = norm(p.activeIngredient)
      const nm = norm(p.name)
      const hit = actives.find((a) => (ai && ai.includes(a)) || (nm && nm.includes(a)))
      if (!hit) return null
      const stock = p.stock == null || p.stock === '' ? null : Number(p.stock)
      return { name: p.name, activeIngredient: p.activeIngredient || hit, stock, unit: p.unit || '', matched: hit }
    })
    .filter(Boolean)

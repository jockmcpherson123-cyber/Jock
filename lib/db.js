...
function rowToFeature(r) {
  return {
    ...
    photo: r.photo || '', source: r.source || 'manual',
...
export async function updateIrrigationFeature(id, patch) {
  ...
  // arc / radius ship in phase22 — strip and retry if those columns aren't there yet.
  ...
  if (res.error && /arc|radius/.test(res.error.message || '')) {
    delete row.arc; delete row.arc_start; delete row.radius

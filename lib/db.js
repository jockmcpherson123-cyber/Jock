...
// Lazy client — created on first use, not at import. This keeps pages that only
// *reference* this module (e.g. the public /tv board during static prerender)
// from constructing a Supabase client before env vars exist, while every
// `supabase.from(...)` call keeps working unchanged at runtime.
let _client = null
...
// Edit one reading (e.g. fix a mistyped Stimp). Only maps the columns given.
export async function updateGreensSpeed(id, patch) {
  const row = {}
  if (patch.speed !== undefined) row.speed = patch.speed ?? null
  if (patch.date !== undefined) row.reading_date = patch.date
  if (patch.notes !== undefined) row.notes = patch.notes || ''
  const { data, error } = await supabase.from('greens_speeds').update(row).eq('id', id).select().single()
  if (error) throw error
  return rowToSpeed(data)
}
...
export async function updateScouting(id, patch) {
  const row = { updated_at: new Date().toISOString() }
  if (patch.area !== undefined) row.area = patch.area
  if (patch.date !== undefined) row.observed_date = patch.date
  ...
  let res = await supabase.from('scouting').update(row).eq('id', id).select().single()
  if (res.error && /updated_at/.test(res.error.message || '')) { // column may not exist
    delete row.updated_at
    res = await supabase.from('scouting').update(row).eq('id', id).select().single()
  }
  if (res.error) throw res.error
  return rowToScout(res.data)
}
...
function rowToPart(r) {
  return {
    ...
    brand: r.brand || '', size: r.size || '', photo: r.photo || '',
    stock: Number(r.stock) || 0, lowStock: Number(r.low_stock) || 0, unit: r.unit || 'each',
    location: r.location || '', supplier: r.supplier || '', price: Number(r.price) || 0,
...
function partToRow(p) {
  ...
  if (p.lowStock !== undefined) row.low_stock = Number(p.lowStock) || 0
  if (p.unit !== undefined) row.unit = p.unit || 'each'
...
export async function fetchParts() {
  const { data, error } = await supabase.from('irrigation_parts').select('*').order('name', { ascending: true })
  if (error) throw error
  return (data || []).map(rowToPart)
}
export async function addPart(p) {
  const { data, error } = await supabase.from('irrigation_parts').insert(partToRow(p)).select().single()
  if (error) throw error
  return rowToPart(data)
}
export async function updatePart(id, patch) {
  const row = partToRow(patch)
  row.updated_at = new Date().toISOString()
  let res = await supabase.from('irrigation_parts').update(row).eq('id', id).select().single()
  if (res.error && /updated_at/.test(res.error.message || '')) { // column may not exist
    delete row.updated_at
    res = await supabase.from('irrigation_parts').update(row).eq('id', id).select().single()
  }
  if (res.error) throw res.error
  return rowToPart(res.data)
}
export async function deletePart(id) {
  const { error } = await supabase.from('irrigation_parts').delete().eq('id', id)
  if (error) throw error
}
...
// ── Course shapes (drawn surfaces + pipe lines on the Course Map) ─────────────
function rowToShape(r) {
  return { id: r.id, shape: r.shape || 'area', kind: r.kind || '', color: r.color || '', points: r.points || [], label: r.label || '', notes: r.notes || '', source: r.source || 'draw', createdAt: r.created_at }
}
export async function fetchShapes() {
  const all = []; const PAGE = 1000
  for (let from = 0; ; from += PAGE) {
    const { data, error } = await supabase.from('course_shapes').select('*').order('created_at', { ascending: true }).range(from, from + PAGE - 1)
    if (error) throw error
    all.push(...(data || []))
    if (!data || data.length < PAGE) break
  }
  return all.map(rowToShape)
}
export async function addShape(s) {
  const row = { shape: s.shape || 'area', kind: s.kind || '', color: s.color || '', points: s.points || [], label: s.label || '', notes: s.notes || '', source: s.source || 'draw' }
  const { data, error } = await supabase.from('course_shapes').insert(row).select().single()
  if (error) throw error
  return rowToShape(data)
}
export async function updateShape(id, patch) {
  const row = { updated_at: new Date().toISOString() }
  ;['shape', 'kind', 'color', 'points', 'label', 'notes'].forEach((k) => { if (patch[k] !== undefined) row[k] = patch[k] })
  const { data, error } = await supabase.from('course_shapes').update(row).eq('id', id).select().single()
  if (error) throw error
  return rowToShape(data)
}
export async function deleteShape(id) {
  const { error } = await supabase.from('course_shapes').delete().eq('id', id)
  if (error) throw error
}

// ── Fertilizer sheets ────────────────────────────────────────────────────────
function rowToFert(r) {
  return {
    id: r.id, area: r.area || '', product: r.product || '', analysis: r.analysis || {},
    rate: r.rate ?? 0, bag: r.bag ?? 50, adjustPct: r.adjust_pct ?? 0, pricePerBag: r.price_per_bag ?? 0,
    applicator: r.applicator || '', appDate: r.app_date || '', status: r.status || 'planned',
    sections: r.sections || [], notes: r.notes || '', createdAt: r.created_at,
  }
}
function fertToRow(f) {
  return {
    area: f.area || '', product: f.product || '', analysis: f.analysis || {},
    rate: Number(f.rate) || 0, bag: Number(f.bag) || 50, adjust_pct: Number(f.adjustPct) || 0,
    price_per_bag: Number(f.pricePerBag) || 0, applicator: f.applicator || '',
    app_date: f.appDate || null, status: f.status || 'planned', sections: f.sections || [], notes: f.notes || '',
  }
}
export async function fetchFertSheets() {
  const { data, error } = await supabase.from('fert_sheets').select('*').order('created_at', { ascending: false })
  if (error) throw error
  return (data || []).map(rowToFert)
}
export async function addFertSheet(f) {
  const { data, error } = await supabase.from('fert_sheets').insert(fertToRow(f)).select().single()
  if (error) throw error
  return rowToFert(data)
}
export async function updateFertSheet(id, patch) {
  const row = { ...fertToRow(patch), updated_at: new Date().toISOString() }
  const { data, error } = await supabase.from('fert_sheets').update(row).eq('id', id).select().single()
  if (error) throw error
  return rowToFert(data)
}
export async function deleteFertSheet(id) {
  const { error } = await supabase.from('fert_sheets').delete().eq('id', id)
  if (error) throw error
}
...
function rowToApplication(r) {
  return {
    ...
    // Model-generated reasoning (chemistry, reason, rate, FRAC, generic, source),
    // present on applications built by "Build from Models".
    model: r.data?.fromModel
      ? { chemistry: r.data.chemistry || '', reason: r.data.reason || '', rate: r.data.rate || '', frac: r.data.frac || '', generic: !!r.data.generic, source: r.data.source || '' }
      : null,
  }
}

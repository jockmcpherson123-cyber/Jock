...
function rowToTask(r) {
  return {
    ...
    equipment: r.equipment || '', status: r.status || 'todo',
    minutes: r.minutes ?? null, sort: r.sort ?? 0, notes: r.notes || '', createdAt: r.created_at,
  }
}
function taskToRow(t) {
  return {
    task_date: t.date, job: t.job, area: t.area || '', assignee: t.assignee || '',
    equipment: t.equipment || '', status: t.status || 'todo',
    minutes: t.minutes === '' || t.minutes == null ? null : Number(t.minutes),
    sort: t.sort ?? 0, notes: t.notes || '',
  }
}
...
export async function addCrewTask(t) {
  const { data, error } = await supabase.from('crew_tasks').insert(taskToRow(t)).select().single()
  if (error) throw error
  return rowToTask(data)
}

export async function addCrewTasks(list) {
  const { error } = await supabase.from('crew_tasks').insert(list.map(taskToRow))
  if (error) throw error
}

export async function updateCrewTask(t) {
  const { data, error } = await supabase.from('crew_tasks').update(taskToRow(t)).eq('id', t.id).select().single()
  if (error) throw error
  return rowToTask(data)
}
...
function soilTestFields(t) {
  return {
    ...
    extras: { ...(t.extras || {}), grasses: t.grasses || [], soilType: t.soilType || '' },
  }
}

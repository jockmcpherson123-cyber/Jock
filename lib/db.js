...
function rowToTask(r) {
  return {
    ...
    minutes: r.minutes ?? null, sort: r.sort ?? 0, notes: r.notes || '', slot: r.slot || '1', createdAt: r.created_at,
  }
}
function taskToRow(t) {
  return {
    ...
    sort: t.sort ?? 0, notes: t.notes || '', slot: t.slot || '1',
  }
}
// The `slot` column (1st/2nd/3rd jobs) ships in phase16.sql. Until that's run,
// strip it so writes still work — everything just reads back as the 1st round.
const withoutSlot = (row) => { const { slot, ...rest } = row; return rest }
const isSlotError = (e) => e && /slot/i.test(e.message || '')
...
export async function addCrewTask(t) {
  ...
  let { data, error } = await supabase.from('crew_tasks').insert(row).select().single()
  if (isSlotError(error)) ({ data, error } = await supabase.from('crew_tasks').insert(withoutSlot(row)).select().single())
  if (error) throw error
...
export async function addCrewTasks(list) {
  ...
  let { error } = await supabase.from('crew_tasks').insert(rows)
  if (isSlotError(error)) ({ error } = await supabase.from('crew_tasks').insert(rows.map(withoutSlot)))
  if (error) throw error
}

export async function updateCrewTask(t) {
  const row = taskToRow(t)
  let { data, error } = await supabase.from('crew_tasks').update(row).eq('id', t.id).select().single()
  if (isSlotError(error)) ({ data, error } = await supabase.from('crew_tasks').update(withoutSlot(row)).eq('id', t.id).select().single())
  if (error) throw error
  return rowToTask(data)
}

...
function rowToFeature(r) {
  return {
    ...
    photo: r.photo || '', source: r.source || 'manual', createdAt: r.created_at,
  }
}
export async function fetchIrrigation() {
  const { data, error } = await supabase.from('irrigation_features').select('*').order('created_at', { ascending: true })
  if (error) throw error
...
export async function addIrrigationFeature(f) {
  ...
  const { data, error } = await supabase.from('irrigation_features').insert(row).select().single()
  if (error) throw error
...
export async function addIrrigationFeatures(list) {
  ...
  for (let i = 0; i < rows.length; i += 500) {
    ...
    const { error } = await supabase.from('irrigation_features').insert(chunk)
    if (error) throw error
...
export async function updateIrrigationFeature(id, patch) {
  ...
  ;['kind', 'lat', 'lng', 'label', 'zone', 'size', 'status', 'notes', 'photo', 'source'].forEach((k) => { if (patch[k] !== undefined) row[k] = patch[k] })
  const { data, error } = await supabase.from('irrigation_features').update(row).eq('id', id).select().single()
  if (error) throw error
  return rowToFeature(data)
}

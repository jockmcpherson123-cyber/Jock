...
export function defaultTrigger(type) {
  if (String(type || '').toLowerCase().includes('growth')) {
    return { mode: 'gdd', base: 32, target: 200 }
  }
  return { mode: 'date' }
}
...
export function normalizeTrigger(t, type) {
  ...
  if (base.mode === 'gdd') {
    ...
    base.target = Number(base.target) > 0 ? Number(base.target) : 200
...
export function triggerStatus(app, ctx = {}) {
  ...
  if (t.mode === 'gdd') {
    const anchor = lastSprayDate(ctx.sheets, app.area, app.product, today) || app.templateDate || app.plannedDate || null

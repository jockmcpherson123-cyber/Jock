...
import { fungicidesFor, ownedMatch, ratingsSourceFor } from './fungicides'
...
function brandName(trade) {
  return String(trade || '').split(/[/,]/)[0].replace(/\(.*?\)/g, '').trim()
}
// A FRAC-diverse rotation of the best-rated fungicides for a disease: top score
// first, one product per FRAC group so consecutive sprays never repeat a mode of
// action. Prefers the professional (Rutgers) source.
function fungicideRotation(diseaseId) {
  const ranked = fungicidesFor(diseaseId, ratingsSourceFor(diseaseId))
  const seen = new Set()
  const rot = []
  for (const r of ranked) {
    if (seen.has(r.frac)) continue
    seen.add(r.frac)
    rot.push(r)
    if (rot.length >= 5) break
  }
  return rot.length ? rot : ranked.slice(0, 5)
}
...
export function buildProgram(year, { areas = [], products = [], climate = null } = {}) {
  ...
  const rotCache = {} // diseaseId → FRAC-diverse ranked product rotation
  ...
  surfaces.forEach((surf) => {
    RULES.forEach((r) => {
      ...
      dates.forEach((date, i) => {
        let product, chemistry, actives, source = r.source, rank = null, lib

        // Rated fungicide → pick a FRAC-rotated top-efficacy product; use the
        // club's own product where it matches, else the trade name.
        if (r.diseaseId) {
          const rotList = (rotCache[r.diseaseId] ||= fungicideRotation(r.diseaseId))
          const pick = rotList.length ? rotList[i % rotList.length] : null
          if (pick) {
            const owned = ownedMatch(pick, products)
            actives = [pick.ai]
            product = owned ? owned.name : brandName(pick.trade)
            chemistry = `FRAC ${pick.frac} · efficacy ${pick.rating}${owned ? ' · in your stock' : ''}`
            rank = pick.score
            source = `${pick.source} efficacy rating`
            lib = owned
              ? [{ name: owned.name, matched: pick.ai, stock: owned.stock == null || owned.stock === '' ? null : Number(owned.stock), unit: owned.unit || '' }]
              : libraryFor(actives, products)
          }
        }

        // Rotation-based (legacy) or class-based rule → name a common brand,
        // preferring an owned product in stock.
        if (!product) {
          const rot = r.sched.rotation ? rotate(r.sched.rotation, i) : null
          actives = rot ? rot.actives : r.actives
          chemistry = rot ? `${rot.group} — e.g. ${rot.actives.slice(0, 2).join(', ')}` : r.chemistry
          lib = libraryFor(actives, products)
          const stockHit = lib.find((l) => l.stock > 0) || lib[0] || null
          product = stockHit ? stockHit.name : (brandFor(actives) || r.product)
        }

        apps.push({
          area: surf.name, surface: surf.surface, cat: r.cat, type: CAT_TYPE[r.cat],
          target: r.target, date, interval: r.sched.interval || null,
          chemistry, actives, reason: r.reason, source, rank,
          product, library: lib,
        })
      })
    })

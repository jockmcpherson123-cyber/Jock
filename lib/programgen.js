...
export function buildProgram(year, { grasses = [], products = [], climate = null } = {}) {
  const y = Number(year)
  const cool = isCoolSeason(grasses)

  // Soil-temperature crossing dates: use the course's own averaged archive when
  // provided (climate), falling back to Bethesda regional normals field by field
  // so a single missing crossing still gets a sensible default.
  const soil = {
  ...
  if (cool) {
    ...
    fung.push(...mkOnce({ dates: chain(y, soil.spring65, '06-30', 28), target: 'Summer patch (spring preventive)', area: 'Greens, tees & Poa fairways',
      chemistry: 'FRAC 3 (DMI) or 11 (QoI) — drench in', actives: ['propiconazole', 'azoxystrobin', 'pyraclostrobin', 'metconazole'],
      reason: `Root infection starts as 2" soil warms through ~65°F (~${soil.spring65}); the ONLY effective window is spring, watered in.`,
    ...
    herb.push(...mkOnce({ dates: once(y, '03-20'), target: 'Crabgrass — pre-emergent (1st)', area: 'Fairways, tees & roughs',
      ...
      reason: `Down BEFORE soil reaches 55°F (~${soil.spring55}, forsythia bloom) — once crabgrass germinates a PRE won't stop it.`,
    ...
    herb.push(...mkOnce({ dates: once(y, '09-25'), target: 'Broadleaf weeds — post-emergent (fall)', area: 'Fairways & roughs',
      ...
      source: 'UMD broadleaf control.' }))
    herb.push(...mkOnce({ dates: once(y, soil.fall70), target: 'Poa annua — fall pre-emergent', area: 'Fairways & roughs (not greens)',
      chemistry: 'HRAC 3 (prodiamine) / 29 (indaziflam)', actives: ['prodiamine', 'indaziflam', 'bensulide'],
      reason: `Poa germinates as soil cools below ~70°F (~${soil.fall70}); a fall PRE on fairways/roughs cuts the fall flush.`,
  ...
  return {
    ...
    soil,
    climate: climate && climate.tuned
      ? { tuned: true, source: climate.source, years: climate.years }
      : { tuned: false, source: 'Bethesda, MD regional normals' },
  }
}

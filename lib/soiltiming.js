...
export const TIMING_WINDOWS = [
  ...
  { id: 'dollarspot', label: 'Dollar spot cover', type: 'watchWarm', threshold: 60, months: [5, 6, 7, 8, 9, 10], note: 'Pressure builds once soil holds ~60°F — keep preventive fungicide cover on and rotate FRAC codes.' },
  { id: 'brownpatch', label: 'Brown patch cover', type: 'watchWarm', threshold: 65, months: [6, 7, 8, 9], note: 'Warm nights + soil ≥65°F — keep preventive fungicide cover on, ease off summer nitrogen.' },
  { id: 'pythium', label: 'Pythium watch', type: 'watchWarm', threshold: 68, months: [6, 7, 8, 9], note: 'High risk once soil is above ~68°F with warm, humid nights — Pythium-specific chemistry.' },
  { id: 'anthracnose', label: 'Anthracnose / heat stress', type: 'watchWarm', threshold: 68, months: [6, 7, 8], note: 'Poa greens under heat stress — raise HOC, keep nitrogen up, keep preventive fungicide on.' },
  { id: 'wetting', label: 'Wetting agent / dry spot', type: 'watchWarm', threshold: 70, months: [6, 7, 8, 9], note: 'Soil ≥70°F and drying — apply wetting agents and hand-water to fight localized dry spot.' },
  // ── Fall ───────────────────────────────────────────────────────────────────
  ...
  { id: 'overseed', label: 'Overseeding window', type: 'coolingFalling', threshold: 75, band: 6, months: [8, 9, 10], note: 'Seed as soil cools through ~70–75°F for reliable fall establishment.' },
  ...
  { id: 'fallfert', label: 'Fall fertilization / recovery', type: 'coolingFalling', threshold: 70, band: 8, months: [9, 10, 11], note: 'Prime rooting and recovery window as soil cools through ~65–75°F.' },
  { id: 'snowmold', label: 'Snow mold preventive', type: 'coolingFalling', threshold: 48, band: 6, months: [10, 11, 12], note: 'Preventive fungicide as soil cools below ~50°F, before consistent snow cover.' },
]
...
export function applicationTimings(soil, trend, date = new Date(), overrides = {}, includeOffseason = false) {
  ...
  return TIMING_WINDOWS
    .map((w) => {
      ...
      return { id: w.id, label: w.label, type: w.type, threshold, note: w.note, status, direction: w.type === 'coolingFalling' ? 'falling' : 'rising', months: w.months }
    })

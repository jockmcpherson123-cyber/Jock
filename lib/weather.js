...
export async function fetchWeather(lat, lng, { pastDays = 92, forecastDays = 14 } = {}) {
  const params = new URLSearchParams({
    ...
    hourly: 'temperature_2m,relative_humidity_2m,dew_point_2m,precipitation,precipitation_probability,wind_speed_10m',
...
export function dailyFromHourly(data) {
  ...
  h.time.forEach((t, i) => {
    ...
    const b = (byDay[day] ||= { tSum: 0, rhSum: 0, dpSum: 0, n: 0, tMax: -Infinity, tMin: Infinity, precip: 0, bpHours: 0, windDayMax: 0, probDayMax: 0 })
    ...
    // Daytime (7am–7pm) wind + rain-probability peaks drive the spray-window rating.
    if (hour >= 7 && hour <= 19) {
...
// Rate a day for spraying from its daytime wind + rain outlook.

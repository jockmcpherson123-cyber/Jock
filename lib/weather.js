...
// US National Blend of Models (NBM) — the NWS's operational forecast blend, and
// what most US phone apps (Apple, the NWS app) track. Open-Meteo's default
// global blend can read a few degrees off it on daily highs, so for US courses
// we overlay NBM on the forecast. Open-Meteo exposes it as `ncep_nbm_conus`.
// Returns [] anywhere it doesn't cover (outside CONUS) or on any error, so the
// caller simply keeps its default forecast.
export async function fetchNbmForecast(lat, lng, forecastDays = 16) {
  try {
    const params = new URLSearchParams({
      latitude: String(lat),
      longitude: String(lng),
    ...
    const res = await fetch(`${FORECAST_URL}?${params.toString()}`)
    if (!res.ok) return []
    return dailyFromForecastBlock(await res.json()).filter((d) => d.tMax != null && d.tMin != null)
  } catch { return [] }
}

// Overlay NBM (or any daily temp source) onto an existing daily series WITHOUT
// dropping the other per-day fields (humidity, ET, wind, tMean) that the disease
// and irrigation models need — unlike mergeDaily, this swaps only the high/low
// and precip on the days the overlay covers.
export function overlayForecastTemps(base, overlay) {
  if (!overlay || !overlay.length) return base
  const m = new Map(overlay.map((d) => [d.date, d]))
  return (base || []).map((d) => {
    const o = m.get(d.date)
    if (!o || o.tMax == null || o.tMin == null) return d
    return { ...d, tMax: o.tMax, tMin: o.tMin, precip: o.precip != null ? o.precip : d.precip, precipProb: o.precipProb != null ? o.precipProb : d.precipProb, code: o.code != null ? o.code : d.code }
  })
}

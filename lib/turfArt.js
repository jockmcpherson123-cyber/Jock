...
// A rotation swirl (two curved arrows) — clockwise or anti-clockwise. Each arc
// ends in a clean arrowhead that caps the line, pointing the way it's turning.
function rotArrows(cx, cy, r, cw, id) {
  const segs = cw ? [[40, 165], [220, 345]] : [[165, 40], [345, 220]]
  const sweep = cw ? 1 : 0
  // arrowhead sized to the stroke (a neat cap), not the disc — the outline is
  // just a touch larger than the gold so it reads as an outlined arrow.
  const hs = Math.max(7, r * 0.42)
  const draw = (col, w, hsz) => segs.map(([s, e]) => {
    // stop the drawn arc a little short of the tip so the arrowhead sits over a
    // clean line end, not past a round cap.
    const eShort = e - (cw ? 8 : -8)
    const [sx, sy] = cp(cx, cy, r, s / 30), [gx, gy] = cp(cx, cy, r, eShort / 30), [ex, ey] = cp(cx, cy, r, e / 30)
    const ang = ((cw ? e : e + 180) * Math.PI) / 180  // tangent = direction of travel
    return `<path d="M${sx.toFixed(2)},${sy.toFixed(2)} A${r} ${r} 0 0 ${sweep} ${gx.toFixed(2)},${gy.toFixed(2)}" fill="none" stroke="${col}" stroke-width="${w}" stroke-linecap="round"/><polygon points="${headPts(ex, ey, ang, hsz)}" fill="${col}"/>`
  }).join('')
  return draw(C.arrowOut, 8, hs + 2.5) + draw(C.gold, 4.4, hs)
}
...
// grain/sun finish. A half & half fairway is split into two halves — top/bottom
// on a horizontal hole, left/right on a vertical one.
function region(pathD, step, cA, cB, band, cx, cy, reach, id, sub, kind, W = 340, H = 200, splitVert = false) {
  const clip = `r${sub}${id}`
  let fill
  if (kind === 'fairway' && step && step.type === 'circle') {
    const cw = step.dir !== 'acw'
    const dark = [C.fairDark1, C.fairDark2], light = [C.fairLight1, C.fairLight2]
    const A = cw ? dark : light, B = cw ? light : dark
    const ac = `ra${sub}${id}`, bc = `rb${sub}${id}`
    const clips = splitVert
      ? `<clipPath id="${ac}"><rect x="0" y="0" width="${cx}" height="${H}"/></clipPath><clipPath id="${bc}"><rect x="${cx}" y="0" width="${W - cx}" height="${H}"/></clipPath>`
      : `<clipPath id="${ac}"><rect x="0" y="0" width="${W}" height="${cy}"/></clipPath><clipPath id="${bc}"><rect x="0" y="${cy}" width="${W}" height="${H - cy}"/></clipPath>`
    fill = `${clips}
      <g clip-path="url(#${ac})">${stripeFill(0, A[0], A[1], band, 0, cx, cy, reach, id)}</g>
  ...
  return `<clipPath id="${clip}"><path d="${pathD}"/></clipPath>
    <g clip-path="url(#${clip})">${fill}
      <rect x="0" y="0" width="${W}" height="${H}" fill="url(#sun${id})"/>
      <rect x="0" y="0" width="${W}" height="${H}" filter="url(#g${id})" opacity="0.3" style="mix-blend-mode:soft-light"/>
    </g>
...
// The same hole, drawn TALL to run vertically up the side of the crew TV — green
// at the top, tees at the bottom. Stripes still point in their true compass
// direction (a 12–6 green is vertical on the tile), so the layout rotates but
// the striping does not.
export function holeSVGVertical({ greenStep, fairStep, apprStep, teeStep, width = 200 } = {}) {
  const id = hash(['vhole', greenStep && greenStep.a, greenStep && greenStep.dir, fairStep && fairStep.dir, fairStep && fairStep.a, apprStep && apprStep.a, teeStep && teeStep.a].join('-'))
  const W = 200, H = 420
  const height = Math.round(width * (H / W))
  const fairway = 'M72,196 C62,224 62,252 70,278 C78,304 64,326 72,346 C78,364 86,376 100,384 C114,376 122,364 128,346 C136,326 122,304 130,278 C138,252 138,224 128,196 C112,188 88,188 72,196 Z'
  const green = 'M56,72 C52,52 74,42 100,42 C130,42 156,54 158,80 C160,106 140,124 108,126 C80,128 60,116 54,94 C52,86 54,80 56,72 Z'
  const collar = 'M52,72 C48,48 72,38 100,38 C132,38 160,51 162,80 C164,109 142,129 108,131 C78,133 56,120 50,96 C48,86 50,80 52,72 Z'
  const approach = 'M80,142 C88,136 112,136 120,142 C128,150 128,182 118,190 C110,197 90,197 82,190 C74,182 74,150 80,142 Z'
  const tee1 = 'M76,360 Q74,356 80,356 L120,356 Q126,356 124,360 L126,380 Q126,384 120,384 L80,384 Q74,384 76,380 Z'
  const tee2 = 'M82,390 Q80,387 85,387 L115,387 Q120,387 118,390 L119,408 Q119,411 114,411 L86,411 Q81,411 82,408 Z'
  return `<svg width="${width}" height="${height}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet" style="display:block;width:100%;height:auto">${defs(id)}
    <rect x="0" y="0" width="${W}" height="${H}" rx="20" fill="#2b6a3c"/>
    <rect x="0" y="0" width="${W}" height="${H}" rx="20" filter="url(#g${id})" opacity="0.5" style="mix-blend-mode:soft-light"/>
    <path d="M174,116 C184,170 166,220 172,272 C178,328 160,360 150,392" fill="none" stroke="#c9b98f" stroke-width="7" stroke-linecap="round" opacity="0.9"/>
    <path d="M174,116 C184,170 166,220 172,272 C178,328 160,360 150,392" fill="none" stroke="#e6dcc0" stroke-width="3" stroke-linecap="round" opacity="0.7"/>
    ${region(fairway, fairStep, C.fairA, C.fairB, 12, 100, 285, 250, id, 'f', 'fairway', W, H, true)}
    ${region(tee1, teeStep, C.greenA, C.greenB, 8, 100, 370, 55, id, 't1', 'tee', W, H)}
    ${region(tee2, teeStep, C.greenA, C.greenB, 8, 100, 399, 55, id, 't2', 'tee', W, H)}
    ${region(approach, apprStep, C.greenA, C.greenB, 8, 100, 166, 70, id, 'a', 'approach', W, H)}
    <path d="${collar}" fill="${C.collar}"/>
    ${region(green, greenStep, C.greenA, C.greenB, 7, 105, 84, 80, id, 'g', 'green', W, H)}
    ${bunker(50, 62, 15, 9, id, 'b1')}
    ${bunker(158, 106, 14, 9, id, 'b2')}
    ${bunker(150, 270, 9, 16, id, 'b3')}
    <ellipse cx="102" cy="74" rx="3" ry="2" fill="${C.arrowOut}"/>
    <line x1="102" y1="73" x2="102" y2="50" stroke="rgba(0,0,0,0.3)" stroke-width="2.8" stroke-linecap="round"/>
    <line x1="102" y1="73" x2="102" y2="50" stroke="#F5F4EF" stroke-width="1.8" stroke-linecap="round"/>
    <path d="M102,50 L120,54 Q114,57 120,60 L102,60.5 Z" fill="#D8402E"/>
    <rect x="0.5" y="0.5" width="${W - 1}" height="${H - 1}" rx="20" fill="none" stroke="#1c4a2b" stroke-width="1"/>
  </svg>`
}

...
export function holeSVGVertical({ greenStep, fairStep, apprStep, teeStep, width = 200 } = {}) {
  ...
  const fairway = 'M74,214 C64,248 64,280 72,312 C80,344 62,372 70,404 C76,432 86,452 100,470 C114,452 126,432 132,404 C140,372 122,344 130,312 C138,280 138,248 128,214 C112,206 90,206 74,214 Z'
  const green = 'M60,70 C56,52 74,42 100,42 C128,42 152,52 154,78 C156,104 138,124 108,126 C82,128 64,116 58,94 C56,86 58,78 60,70 Z'
  const collar = 'M56,70 C52,48 72,38 100,38 C130,38 156,49 158,78 C160,107 140,129 108,131 C80,133 60,120 54,96 C52,87 54,79 56,70 Z'
  const approach = 'M80,150 C88,144 112,144 120,150 C128,158 128,198 118,208 C110,215 90,215 82,208 C74,198 74,158 80,150 Z'
  const tee1 = 'M74,470 Q72,466 78,466 L122,466 Q128,466 126,470 L128,494 Q128,498 122,498 L78,498 Q72,498 74,494 Z'
  const tee2 = 'M80,506 Q78,503 83,503 L117,503 Q122,503 120,506 L121,528 Q121,531 116,531 L84,531 Q79,531 80,528 Z'
  return `<svg width="${width}" height="${height}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMidYMid meet" style="display:block;width:100%;height:auto">${defs(id)}
    ...
    <path d="M176,120 C188,190 166,250 174,330 C182,410 162,460 150,506" fill="none" stroke="#c9b98f" stroke-width="7" stroke-linecap="round" opacity="0.9"/>
    <path d="M176,120 C188,190 166,250 174,330 C182,410 162,460 150,506" fill="none" stroke="#e6dcc0" stroke-width="3" stroke-linecap="round" opacity="0.7"/>
    ${region(fairway, fairStep, C.fairA, C.fairB, 12, 100, 340, 320, id, 'f', 'fairway', W, H, true)}
    ${region(tee1, teeStep, C.greenA, C.greenB, 8, 100, 482, 60, id, 't1', 'tee', W, H)}
    ${region(tee2, teeStep, C.greenA, C.greenB, 8, 100, 517, 60, id, 't2', 'tee', W, H)}
    ${region(approach, apprStep, C.greenA, C.greenB, 8, 100, 178, 80, id, 'a', 'approach', W, H)}
    <path d="${collar}" fill="${C.collar}"/>
    ${region(green, greenStep, C.greenA, C.greenB, 7, 105, 84, 80, id, 'g', 'green', W, H)}
    ${bunker(52, 62, 15, 9, id, 'b1')}
    ${bunker(156, 108, 14, 9, id, 'b2')}
    ${bunker(150, 300, 9, 17, id, 'b3')}

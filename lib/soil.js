...
// Base-saturation ideal bands (percent of CEC held by each cation), per the
// classic Logan/BCSR ranges shown on the report. MLSN doesn't use these — they
// are informational — but supers like to eyeball them.
export const BASE_SAT_BANDS = [
  { key: 'ca', label: 'Calcium', lo: 60, hi: 70 },
  { key: 'mg', label: 'Magnesium', lo: 10, hi: 20 },
  { key: 'k', label: 'Potassium', lo: 2, hi: 5 },
  { key: 'na', label: 'Sodium', lo: 0, hi: 3 },
  { key: 'h', label: 'Hydrogen', lo: 10, hi: 15 },
]

// Read a set of base-saturation percentages against the ideal bands. Returns one
// row per entered value flagged low / ok / high (for sodium, "high" is the one
// that matters).
export function baseSaturation(bs = {}) {
  return BASE_SAT_BANDS.map(({ key, label, lo, hi }) => {
    const v = bs[key]
    if (v === '' || v == null || isNaN(Number(v))) return { key, label, value: null, status: 'notest', lo, hi }
    const n = Number(v)
    let status = 'ok'
    if (n < lo) status = 'low'
    else if (n > hi) status = 'high'
    return { key, label, value: n, status, lo, hi }

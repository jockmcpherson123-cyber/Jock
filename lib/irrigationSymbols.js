...
export function symbolSvg(sym, px = 22, rot = 0) {
  if (!sym) return ''
  ...
  // Part-circle heads (and anything else) can be aimed by rotating the shape.
  // The glyph (letters) is drawn after, unrotated, so P/W/SH stay upright.
  const g = rot ? `<g transform="rotate(${rot} ${c} ${c})">${shape}</g>` : shape
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${s}" height="${s}" viewBox="0 0 ${s} ${s}">${g}${glyph}</svg>`
}

// True for the part-circle head symbols, which can be aimed.
export const isPartCircle = (id) => (SYMBOLS[id]?.shape === 'half')

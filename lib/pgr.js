...
// Turf DMI (FRAC 3) active ingredients — used when a product's FRAC code isn't
// filled in. All are demethylation inhibitors that suppress growth. Nearly every
// DMI ends in "-conazole", so we catch that pattern too (below) — this list is
// the handful that don't, plus common ones for clarity.
const DMI_ACTIVES = [
  'propiconazole', 'tebuconazole', 'myclobutanil', 'triadimefon', 'triadimenol', 'metconazole',
  ...
  'cyproconazole', 'epoxiconazole', 'hexaconazole', 'fenbuconazole', 'bitertanol',
  'triflumizole', 'imazalil', 'prochloraz', 'triforine', 'pyrifenox',
]
// Anything ending in "-conazole" is a FRAC 3 DMI — future-proofs new products.
const isConazole = (ai) => /conazole\b/.test(ai)
...
export function isDMI(product) {
  ...
  return isConazole(ai) || DMI_ACTIVES.some((a) => ai.includes(a))
}

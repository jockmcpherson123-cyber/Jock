...
import { fetchWeather, dailyFromHourly, summarize } from '@/lib/weather'
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        if (!cancelled) setState({ loading: false, error: null, daily, summary })
      } catch (e) {
  ...
  return (
    <div className="pt-6 pb-10 space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Stat icon={<TrendingUp size={15} />} label="GDD (base 50)" value={Math.round(summary.gddNow).toLocaleString()} accent={FOREST} />
      </div>
      ...
      <p className="font-body text-[11px] text-slate-400">
        Live from Open-Meteo for your location. Automatic daily history and per-area soil-temp models (Pythium, pre-emergent windows) come next.
      </p>
    </div>
  )
}
